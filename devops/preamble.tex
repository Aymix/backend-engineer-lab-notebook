...
% =====================================================================
%  THE DEVOPS ENGINEER'S LAB NOTEBOOK  —  shared design system
%  Companion workbook to the "DevOps Engineer 14-Day Speedrun Roadmap"
%  Volume II of the Speedrun Series (Volume I: Backend Engineering)
...
\definecolor{cBrand}{HTML}{0E4A5B}      % deep petrol teal — primary (Vol. II)
\definecolor{cBrandDeep}{HTML}{07303C}  % darker petrol (headers)
\definecolor{cAccent}{HTML}{E8952E}     % warm amber — series accent
...
\definecolor{cSenior}{HTML}{0F766E}     % teal-green — senior engineer insight
...
\definecolor{cDb}{HTML}{6D28D9}         % purple — (legacy alias)
...
% --- DevOps volume: additional semantic colours ---
\definecolor{cInfra}{HTML}{2A5A8C}      % slate blue — infrastructure insight
\definecolor{cCost}{HTML}{8A6D0B}       % olive gold — cloud cost optimization
\definecolor{cAuto}{HTML}{6D28D9}       % purple — automation tip
\definecolor{cRely}{HTML}{1B6E4A}       % forest green — reliability principle
\definecolor{cTrade}{HTML}{92400E}      % brown — design tradeoff
\definecolor{cScale}{HTML}{075985}      % deep blue — scalability note
\definecolor{cOps}{HTML}{334155}        % slate — operational excellence
\definecolor{cDR}{HTML}{9D174D}         % crimson — disaster recovery
...
\newtcblisting{yamlbox}[1][config.yaml]{wbcode, listing style=stYaml, title={\ #1\ }}
...
% ---- DevOps volume: Terraform (HCL), Kubernetes manifests, terminal output ----
\lstdefinelanguage{hclx}{
  morekeywords={resource,provider,variable,output,module,data,locals,terraform,backend,
    required_providers,for_each,count,depends_on,dynamic,lifecycle,true,false,null,var,each,source},
  keywordstyle=\color{lstKw}\bfseries,
  keywordstyle=[2]\color{lstKw2},
  morekeywords=[2]{string,number,bool,list,map,object,set,any},
  comment=[l]{\#}, morecomment=[l]{//}, morecomment=[s]{/*}{*/},
  morestring=[b]",
  sensitive=true,
}
\lstdefinestyle{stHcl}{language=hclx}
\newtcblisting{tfbox}[1][main.tf]{wbcode, listing style=stHcl, title={\ #1\ }}

% Kubernetes manifest — YAML styling, manifest-flavoured default caption
\newtcblisting{k8sbox}[1][deployment.yaml]{wbcode, listing style=stYaml, title={\ #1\ }}
% Ansible playbook — YAML styling
\newtcblisting{ansiblebox}[1][playbook.yml]{wbcode, listing style=stYaml, title={\ #1\ }}

% Terminal / log output — no keyword colouring, but +/-/~ diff markers and log levels highlighted.
\definecolor{lstAdd}{HTML}{86E08A}
\definecolor{lstDel}{HTML}{FF8A8A}
\definecolor{lstChg}{HTML}{F0C674}
\lstdefinelanguage{outx}{
  morekeywords={},
  moredelim=[l][\color{lstAdd}]{+\ },
  moredelim=[l][\color{lstDel}]{-\ },
  moredelim=[l][\color{lstChg}]{\~\ },
  morecomment=[l][\color{lstCom}\itshape]{\#},
}
\lstdefinestyle{stOut}{language=outx, basicstyle=\ttfamily\scriptsize\color{lstTxt}}
\newtcblisting{outbox}[1][terminal output]{wbcode, listing style=stOut, title={\ #1\ }}
% Terraform plan diff — same output style, plan-flavoured caption
\newtcblisting{planbox}[1][terraform plan]{wbcode, listing style=stOut, title={\ #1\ }}

% ---- AWS / cloud console mock frame (illustration placeholder, not a screenshot) ----
\newtcolorbox{consolemock}[1][AWS Management Console]{enhanced, breakable=false,
  colback=white, colframe=cInk!35, boxrule=0.7pt, arc=1.4mm,
  left=3mm, right=3mm, top=3mm, bottom=2.6mm,
  fonttitle=\sffamily\bfseries\scriptsize, coltitle=white, colbacktitle=cInk!78,
  title={\faCloud\ \ #1 \hfill \normalfont\scriptsize\textit{illustration}},
  fontupper=\footnotesize}
% a labelled console "field" row
\newcommand{\consolerow}[2]{\noindent{\sffamily\scriptsize\color{cInk!60}#1}\;\;{\ttfamily\scriptsize\color{cInk}#2}\par\addvspace{0.7mm}}
...
\definecallout{seniornote}{cSenior}{\faLightbulb\enspace Senior engineer insight}
...
% --- DevOps volume: additional engineering notes ---
\definecallout{infranote}{cInfra}{\faServer\enspace Infrastructure insight}
\definecallout{costnote}{cCost}{\faCoins\enspace Cloud cost optimization}
\definecallout{autotip}{cAuto}{\faRobot\enspace Automation tip}
\definecallout{reliability}{cRely}{\faHeartbeat\enspace Reliability principle}
\definecallout{tradeoff}{cTrade}{\faBalanceScale\enspace Design trade-off}
\definecallout{scalenote}{cScale}{\faChartLine\enspace Scalability note}
\definecallout{opsnote}{cOps}{\faTasks\enspace Operational excellence}
\definecallout{drtip}{cDR}{\faLifeRing\enspace Disaster recovery tip}
...
\tikzset{
  ...
  % --- DevOps volume: infrastructure diagram vocabulary ---
  wbboxcloud/.style={wbbox, draw=cInfra, fill=cInfra!8},          % AWS / cloud resource
  wbboxk8s/.style={wbbox, draw=cScale, fill=cScale!8},            % Kubernetes object
  wbboxsec/.style={wbbox, draw=cSecurity, fill=cSecurity!7},      % security boundary/control
  wbboxops/.style={wbbox, draw=cOps, fill=cOps!8},                % operational component
  % dashed grouping boundary — VPC, subnet, AZ, namespace, cluster
  wbzone/.style={draw=cInfra!70, line width=0.8pt, dashed, rounded corners=2.4mm,
    inner sep=3.4mm, fill=cInfra!3},
  wbzonesec/.style={wbzone, draw=cSecurity!60, fill=cSecurity!3},
  wbzonek8s/.style={wbzone, draw=cScale!60, fill=cScale!3},
  % label sitting on a zone boundary
  wbzonelabel/.style={font=\sffamily\bfseries\scriptsize, text=cInfra, fill=cFigBg, inner sep=1.2pt},
  wbzonelabelk8s/.style={wbzonelabel, text=cScale},
  wbzonelabelsec/.style={wbzonelabel, text=cSecurity},
  % dotted "traffic flow" arrow, for request paths
  wbflow/.style={-{Latex[length=2.6mm,width=2.1mm]}, line width=0.9pt, draw=cAccent, densely dashed},
}
...
\newcommand{\labwhy}[1]{\par\addvspace{0.8mm}\noindent{\sffamily\itshape\color{cLab}\faLightbulb\ Why:\ }{\small\itshape\color{cInk!85}#1}\par\addvspace{0.4mm}}
...
\newtcolorbox{buildalong}[1][CloudBase — extend the platform]{enhanced,breakable,
...
\fancyhead[R]{\footnotesize\sffamily\color{cInk!58}The DevOps Engineer's Lab Notebook}
